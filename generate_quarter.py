...
def apply_rotation_to_props(R, note_props):
    ...
    for idx in note_props:
        ...
        note_props[idx]['hole_positions_pan'] = [
            R @ bp for bp in note_props[idx]['hole_positions_pan']
        ]
...
def remove_small_components(vertices, faces, face_attrs=None, min_face_ratio=0.005):
    """Remove small disconnected mesh fragments, keeping only components
    with at least min_face_ratio of total faces.

    Applied to the playing surface BEFORE combining with supports,
    so only stray lozenges from hole cutting get removed.

    Returns (vertices, faces, face_attrs) — all reindexed.
    """
    n_faces = len(faces)
    if n_faces == 0:
        return vertices, faces, face_attrs
    ...
    for start_fi in range(n_faces):
        ...
        while queue:
            ...
            component.append(fi)
            for vi in faces[fi]:
                for neighbor_fi in vert_to_faces.get(vi, []):
    ...
    threshold = max(1, int(n_faces * min_face_ratio))
    keep_idx = []
    removed = 0
    ...
    if removed > 0:
        print(f"    Removed {removed} faces in {len(components) - len([c for c in components if len(c) >= threshold])} floating fragment(s)")
        keep_idx.sort()
        new_faces = [faces[fi] for fi in keep_idx]
        new_attrs = [face_attrs[fi] for fi in keep_idx] if face_attrs is not None else None
        new_v, new_f = reindex_mesh(vertices, new_faces)
        return new_v, new_f, new_attrs
...
def thicken_with_pocket(vertices, faces, is_pocket_face, thickness, pocket_depth):
    ...
    all_f = top_f + bot_f + side_f

    # Close small interior holes created by normal-offset divergence
    # at grove boundaries where surface curvature changes rapidly.
    all_v, all_f = _close_small_holes(all_v, all_f, max_perimeter=50.0)

    return all_v, all_f


def _close_small_holes(vertices, faces, max_perimeter=50.0):
    """Close small boundary holes by filling connected components of boundary edges.

    Uses connected-component analysis instead of greedy loop traversal,
    which correctly handles T-junctions and branching boundary topologies.
    """
    ...
    for face in faces:
        nf = len(face)
        for i in range(nf):
            e = tuple(sorted([face[i], face[(i + 1) % nf]]))
            edge_count[e] += 1

    boundary_edges = [(v1, v2) for (v1, v2), c in edge_count.items() if c == 1]
    ...
    # For each small component, collect its boundary edges and fill
    fill_faces = []
    n_closed = 0
    boundary_edge_set = set(tuple(sorted(e)) for e in boundary_edges)

    for comp_verts in components:
        # Collect boundary edges in this component
        comp_edges = []
        comp_perim = 0.0
        for e in boundary_edge_set:
            if e[0] in comp_verts:
                d = np.linalg.norm(vertices[e[0]] - vertices[e[1]])
                comp_edges.append(e)
                comp_perim += d

        if comp_perim > max_perimeter:
            continue  # large component = section boundary

        if len(comp_edges) < 3:
            continue

        # Fill each boundary edge with a triangle to the centroid
        pts = vertices[list(comp_verts)]
        center = pts.mean(axis=0)
        ci = len(vertices)
        vertices = np.vstack([vertices, center.reshape(1, 3)])

        for v1, v2 in comp_edges:
            fill_faces.append([v1, v2, ci])
        n_closed += 1

    if n_closed > 0:
        print(f"    Closed {n_closed} small hole(s)")
        faces = faces + fill_faces
...
def _build_y_cap_grid(verts, cell_size=5.0):
    """Build a grid of max Y values from the section's shell vertices.
    Returns dict mapping (grid_x, grid_z) -> max_y."""
    grid = {}
    for v in verts:
        gx = int(round(v[0] / cell_size))
        gz = int(round(v[2] / cell_size))
        key = (gx, gz)
        if key not in grid or v[1] > grid[key]:
            grid[key] = float(v[1])
    return grid, cell_size


def _lookup_y_cap(grid_info, x, z, fallback, search=2):
    """Look up the max Y near (x, z) from the grid, searching nearby cells."""
    grid, cell_size = grid_info
    gx = int(round(x / cell_size))
    gz = int(round(z / cell_size))
    best = None
    for dx in range(-search, search + 1):
        for dz in range(-search, search + 1):
            val = grid.get((gx + dx, gz + dz))
...
def prepare_shells(bowl_verts, bowl_faces, face_material, note_props):
    ...
    # NOTE: Hole cutting (mount + boss) moved to extract_section() so each
    # section only gets holes for its own notes, avoiding bleed-through from
    # adjacent sections' boss holes near the cut boundary.
...
def extract_section(section_id, ps_surf_v, ps_surf_f, ps_is_pocket,
    ...
    # Cut holes for THIS section's notes only (mount + boss through-holes)
    print(f"    Cutting holes for {note_names}...")
    for nname in note_names:
        ...
        normal = props['normal']
        bosses = props['hole_positions_pan']

        ps_f, ps_cut_pocket = remove_faces_in_circle(
            ps_v, ps_f, ps_cut_pocket,
    ...
    ps_v, ps_f = reindex_mesh(ps_v, ps_f)
    print(f"    After holes: {len(ps_f)} faces")
    ...
    # Remove floating fragments from thickened playing surface
    # (boss holes near section boundaries create small disconnected lozenges
    #  that become 3D pieces after thickening)
    ps_v, ps_f, _ = remove_small_components(ps_v, ps_f)
    print(f"    After fragment removal: {len(ps_v)} verts, {len(ps_f)} faces")
    ...
    # Build a lookup of max Y across the section's shell (playing surface +
    # drum wall) so we can clip wall tops to the actual local surface height.
    # We sample a grid in X-Z and record the max Y at each cell.
    section_top_y = _build_y_cap_grid(s_verts, cell_size=5.0)
    ...
    for cut_a in [angle_start, effective_end]:
        ...
        # Clip profile to the actual section surface height at each point
        cut_rad = math.radians(cut_a)
        cut_dx = math.cos(cut_rad)
        cut_dz = math.sin(cut_rad)
        clipped_profile = []
        for r, y in profile:
            x = r * cut_dx
            z = r * cut_dz
            local_cap = _lookup_y_cap(section_top_y, x, z, fallback=y_cap)
            clipped_profile.append((r, min(y, local_cap)))
