...
def load_notepad_properties(props_path, pan_centroid_offset):
    ...
    Returns dict: note_index -> {
        ...
        'normal': np.ndarray(3,),
        'hole_positions_pan': list of np.ndarray(3,),
        'ring': str,
        'pan_object': str,
        'grove_object': str,
        ...
    ...
    for p in props_list:
        ...
        boss_pan = []
        for key in ('hole_positions', 'boss_positions'):
            if key in p:
                for bp in p[key]:
...
def generate_sector(note_index, bowl_vertices, bowl_faces, face_assignments,
    ...
    normal = props['normal']
    boss_positions = props['hole_positions_pan']
    ...
    return {
        ...
        'hole_positions_pan': [bp.tolist() for bp in boss_positions],
...
def main():
    ...
    no_boss = [idx for idx, p in note_props.items() if not p['hole_positions_pan']]
    if no_boss:
        print(f"  WARNING: {len(no_boss)} notes missing hole_positions: {no_boss}")
