...
if len(user_args) >= 2:
    ...
    base = os.path.splitext(INPUT)[0] + "_solid"
OUT_OBJ = os.path.abspath(base + ".obj")
OUT_STL = os.path.abspath(base + ".stl")
print(f"Input:  {INPUT}")
print(f"Output: {OUT_OBJ}\n        {OUT_STL}")
THICKNESS = 5.0   # mm, inward
OFFSET = 1.0       # +1 = original mesh stays on outer surface, thickens inward
SUBDIV_LEVELS = 2  # simple subdivision rounds before solidify
CS_FACTOR = 1.0
CS_ITERATIONS = 5
CS_SCALE = 0.5
